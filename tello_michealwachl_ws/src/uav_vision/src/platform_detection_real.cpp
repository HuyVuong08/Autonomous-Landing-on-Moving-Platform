...
#include <uav_vision/TelloStatus.h>
...
Platform_Detection::Platform_Detection()
{
    ...
    //For Tello
    std::string tello_altitude_topic_;
    ...
    //For Tello
    nh_.param("image_topic", image_topic_, std::string("/tello/camera/image_raw"));
    nh_.param("cam_info_topic", camera_info_topic_, std::string("/tello/camera/camera_info"));
    nh_.param("platform_position_topic", platform_position_topic_, std::string("/platform_xl/estimated_position"));
    ...
    altitude_ = nh_.subscribe(tello_altitude_topic_, 1, &Platform_Detection::height, this);
...
{
    altitude_quadrotor = msg->height_m; 
}


void Platform_Detection::angle(const sensor_msgs::ImuConstPtr& imu_msg)
{

  angle_x               = imu_msg->orientation.x;
  angular_velocity_x    = imu_msg->angular_velocity.x;
 
  angle_y               = imu_msg->orientation.y;
...
void Platform_Detection::colorDetectionCallback(const sensor_msgs::ImageConstPtr& original_image)
{
    ...
    for (int i = 0; i < contours.size(); i++)
    {
        ...
        ROS_INFO("Center of mass in pixels of the target: (%f , %f)", mc.x, mc.y);
        ROS_INFO("The position of the trget is: (%f , %f, %f) in meters with respect to the camera", centroid.pose.position.x, centroid.pose.position.y, centroid.pose.position.z);
