...
class TelloController(object):
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/botello/goal_in_tag", TransformStamped, self.cb_target_pose, queue_size=1)
        rospy.Subscriber("/botello/base_link_in_tag", TransformStamped, self.cb_odom, queue_size=1)
        ...
        self.pub_target = rospy.Publisher(self.ns_node+'target_pose', TransformStamped, queue_size=1)
    ...
    def cb_odom(self, msg):
        if self.slam_control == False:
            ...
            self.current_pose = msg.transform
            # inverse yaw
            self.current_pose.rotation.w = -self.current_pose.rotation.w
            current_orientation = self.current_pose.rotation
            current_orientation_list = [current_orientation.x, current_orientation.y, current_orientation.z,
                                       current_orientation.w]
            current_angles = euler_from_quaternion(current_orientation_list)
            self.current_yaw = current_angles[2]
    ...
    def calculate_control(self):
        ...
        x_vel = self.pid_x.update(self.current_pose.translation.x, self.target_position.transform.translation.x)
        ...
        y_vel = self.pid_y.update(self.current_pose.translation.y, self.target_position.transform.translation.y)
        self.fly_msg.linear.y = y_vel
        #self.fly_msg.linear.y = 0
        ...
        z_vel = self.pid_z.update(self.current_pose.translation.z, self.target_position.transform.translation.z)
        self.fly_msg.linear.z = z_vel
        #self.fly_msg.linear.z = 0
        # calculate PID control value for Tello yaw control
        # target_orientation = self.target_position.pose.orientation
        target_orientation = self.target_position.transform.rotation
        ...
        if self.log_controller:
            self.log_controller.current_x.append(deepcopy(self.current_pose.translation.x))
            self.log_controller.target_x.append(deepcopy(self.target_position.transform.translation.x))
            self.log_controller.current_y.append(deepcopy(self.current_pose.translation.y))
            self.log_controller.target_y.append(deepcopy(self.target_position.transform.translation.y))
            self.log_controller.current_z.append(deepcopy(self.current_pose.translation.z))
            self.log_controller.target_z.append(deepcopy(self.target_position.transform.translation.z))
            self.log_controller.current_yaw.append(deepcopy(self.current_yaw))
            self.log_controller.target_yaw.append(deepcopy(target_angles[2]))
            self.log_controller.error_x.append(deepcopy(self.pid_x.error))
            self.log_controller.error_y.append(deepcopy(self.pid_y.error))
    ...
    def control_iterator(self):
        ...
        # fly to a single target position
            ...
            print("Drone position: ", self.current_pose,"\n")
