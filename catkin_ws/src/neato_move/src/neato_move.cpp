...
NeatoMove::NeatoMove() {
    ...
    takeoff_sub_ = nh_.subscribe("/neato_run", 1, &NeatoMove::takeoffCallback, this);
...
void NeatoMove::takeoffCallback(const std_msgs::EmptyConstPtr & takeoff_signal) {
    ROS_INFO("Starting!");
    start = true;
}

double NeatoMove::doTrajectory() {
...
void NeatoMove::stop(double interval) {
    ROS_INFO("Stopping Neato...");
    twist.linear.x = 0.0;
    twist.angular.z = 0.0;
...
void NeatoMove::advance(double interval) {
...
void NeatoMove::reverse(double interval) {
...
double NeatoMove::doBackAndForth() {
    double interval;
    if (advancing) {
...
{
    ros::init(argc, argv, "neato_move", ros::init_options::NoSigintHandler);
    ROS_INFO("Neato Moves");
    NeatoMove neato_move;
    ...
    while (!g_request_shutdown) {
        usleep(sleep_for_seconds * 1000000); // in microseconds
        sleep_for_seconds = neato_move.doTrajectory();
    }
